...
def get_dashboard_data():
    ...
    # The 'with' statement ensures the database connection is always closed properly.
    with sqlite3.connect(DB_NAME) as conn:
        ...
        if not jobs_df.empty:
            # CORRECTED: Simple string concatenation for DataFrame column.
...
try:
    ...
    # --- Danger Zone (Corrected with Session State) ---
    st.divider()
    ...
    st.error(f"An error occurred: {e}. It's possible the database is empty. Please go to the 'Upload' page to add data.")